...
/** @file db_field_log.h
 * @brief Simple native types logged by `db_post_events()`
 *  
 * This is done for reliable interprocess communication, for any type 
 * which is not a string or an array currently. Other types get whatever 
 * happens to be there when the lower priority task pending on the event 
 * queue wakes up. Strings would slow down events for more reasonable size 
 * values. DB fields of native type string will most likely change 
 * infrequently.
 *
 ...
 *
 *  1. `dbfl_type_ref` - Reference to value
 ...
 *
 *  2. `dbfl_type_val` - Internal value
 *  Used to store small scalar data.  Meta-data and value are present in 
 *  this structure and no external references are used.
...
union native_value {
 ...
 struct db_field_log; 
/*!<  Structure to log the state of a data base field at the
...
typedef enum dbfl_context {
    dbfl_context_read, /*!< read = read reply */
    dbfl_context_event /*!< event = subscription update */
} dbfl_context; /*!< Context of db_field_log */
